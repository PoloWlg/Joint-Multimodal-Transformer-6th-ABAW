...
class CRF(nn.Module):
    ...
    def __init__(self,
        ...
        self.mask = Variable(torch.diag(torch.ones(num_classes)),
                             requires_grad=False)
        self.psi_0 = nn.Parameter(
            (1 / np.sqrt(num_classes)) * torch.randn(num_classes, num_classes))
        self.psi_1 = nn.Parameter(
            (1 / np.sqrt(num_classes)) * torch.randn(num_classes, num_classes))
    ...
    def forward(self, x):
        ...
        for i in range(self.num_updates):
            ...
            zeros_contrib = torch.matmul(1 - q,
                                         torch.t((1. - self.mask) * self.psi_0))
            ones_contrib = torch.matmul(q,
                                        torch.t((1. - self.mask) * self.psi_1))
...
class CRF_pairwise_cond(nn.Module):
    ...
    def forward(self, x, psi_0, psi_1):
        ...
        for i in range(self.num_updates):
            ...
            zeros_contrib = torch.einsum('btj,btij->bti',
                                         (q, (1. - self.mask) * psi_0))
            ones_contrib = torch.einsum('btj,btij->bti',
                                        (q, (1. - self.mask) * psi_1))
...
class InceptionI3d(nn.Module):
    ...
    def __init__(self, num_classes=400, spatial_squeeze=True,
        ...
        self.end_points[end_point] = Unit3D(in_channels=in_channels,
                                            ...
                                            name=name + end_point)  # stride=(2, 2, 2)
        ...
        if self.use_crf:
            if not self.pairwise_cond_crf:
                end_point = 'CRF'
                self.crf = CRF(num_updates=self.num_updates_crf,
                               num_classes=self._num_classes, name='crf')
            else:
                self.psi_0 = Unit3D(in_channels=384 + 384 + 128 + 128,
                                    output_channels=self._num_classes ** 2,
                                    kernel_shape=[1, 1, 1],
                                    padding=0,
                                    activation_fn=None,
                                    use_batch_norm=False,
                ...
                self.psi_1 = Unit3D(in_channels=384 + 384 + 128 + 128,
                                    output_channels=self._num_classes ** 2,
                                    kernel_shape=[1, 1, 1],
                                    padding=0,
                                    activation_fn=None,
                ...
                end_point = 'CRF'
                self.crf = CRF_pairwise_cond(num_updates=self.num_updates_crf,
    ...
    def forward(self, x):
        ...
        if not self.use_crf:  # no CRF
            ...
            # fully-CRF
                ...
                psi_0 = psi_0.reshape(
                    list(logits.permute(0, 2, 1).size()) + [self._num_classes])
                psi_1 = psi_1.reshape(
                    list(logits.permute(0, 2, 1).size()) + [self._num_classes])
