...
def Test(val_loader,
	...
	for key in pred_a.keys():
		label_file_path = os.path.join(dir_out, key + ".txt")
		...
		n = text_file.write("image_location,valence,arousal")
		n = text_file.write('\n')

		for i in range(len(smoothened_preds_a)):
			str_data = ','.join([str(key + '/' + "{:05d}".format(i+1) + '.jpg'),
								 str("{0:.5f}".format(smoothened_preds_v[i])),
								 str("{0:.5f}".format(smoothened_preds_a[i]))])
